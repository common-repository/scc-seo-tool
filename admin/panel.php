 ...
 * Admin Page
 * ============================================================================= */
abstract class sccstslugDP_Admin_Panel {

    protected $layout_like = 'postnew'; // postnew, dashboard, settings
    ...
    function sccstslugDP_Admin_Panel() {
        $this->__construct();
    }
    ...
    function meta_box($object, $box) {
        ...
        $new_fields = sccstslugDP_instance_fields($defaults);
        sccstslugDP_form_fields($new_fields);
    }
    ...
    function update() {
        ...
        if (isset($_POST['save'])) {
            ...
            wp_redirect(add_query_arg($args, sccstslug_get_currenturl(false)));
            exit();
        }
    ...
    function admin_notices() {
        ...
        if (!empty($_GET['updated']) && $parent_file != 'options-general.php')
            echo '<div id="message" class="updated"><p><strong>' . __('Đã lưu lại.', 'dp') . '</strong></p></div>';

        elseif (!empty($_GET['reset']))
            echo '<div id="message" class="updated"><p><strong>' . __('Đã làm lại.', 'dp') . '</strong></p></div>';
    ...
    function page_buttons($in_top = false) {
        if (in_array('save', $this->buttons))
            echo '<input type="submit" name="save" value="' . __('Lưu lại', 'dp') . '" class="button-primary"> ';

        if (in_array('reset', $this->buttons))
            echo '<input type="submit" value="' . __('Làm lại', 'dp') . '" name="reset" class="reset button-highlighted"> ';
    ...
    function defaults($fields = array()) {
        ...
        foreach ($fields as $box_id => $box_fields) {
            foreach (sccstslugDP_field_options($box_fields) as $name => $field) {
                $defaults[$name] = $field;
            }
        }
    ...
    function default_scripts() {
        ...
        wp_enqueue_script('dp-admin', $panel_link_folder. '/admin/js/admin.js', array('jquery'), '', true);
    }
    ...
    function default_styles() {
        ...
        wp_enqueue_style('dp-admin', $panel_link_folder . '/admin/css/admin.css', false);
    }

}

//function dp_register_panel($class) {
//    sccstslugDP_Admin_Panel::register($class);
//}
//
//function dp_unregister_panel($class) {
//    sccstslugDP_Admin_Panel::unregister($class);
//}
...
abstract class sccstslugDP_Post_Panel {
    ...
    function sccstslugDP_Post_Panel() {
        $this->__construct();
    }
    ...
    function meta_box($object, $box) {
        ...
        $new_fields = sccstslugDP_instance_fields($defaults, 'post_meta');
        
        wp_nonce_field($this->nonce_action, $this->nonce);
        echo sccstslugDP_form_fields($new_fields);
    }
    ...
    function handle($post_id, $post) {
        ...
        foreach (sccstslugDP_field_options($fields) as $name => $field) {
...
//    sccstslugDP_Post_Panel::register($class);
//}
//
//function dp_unregister_post_panel($class) {
//    sccstslugDP_Post_Panel::unregister($class);
//}
...
abstract class sccstslugDP_Term_Panel {
    ...
    function sccstslugDP_Term_Panel() {
        $this->__construct();
    }
    ...
    function meta_box($term, $taxonomy) {
        ...
        $new_fields = sccstslugDP_instance_fields($defaults, 'term_meta', $term);
...
//    sccstslugDP_Term_Panel::register($class);
//}
//
//function dp_unregister_term_panel($class) {
//    sccstslugDP_Term_Panel::unregister($class);
//}
...
abstract class sccstslugDP_User_Panel {
    ...
    function sccstslugDP_User_Panel() {
        $this->__construct();
    }
    ...
    function meta_box($user) {
        ...
        $new_fields = sccstslugDP_instance_fields($defaults, 'user_meta', $user);
...
//    sccstslugDP_User_Panel::register($class);
//}
//
//function dp_unregister_user_panel($class) {
//    sccstslugDP_User_Panel::unregister($class);
//}
 ...
 */
//add_filter('set-screen-option', 'seosst_set_screen_option_filter', 999, 3);
//
//function seosst_set_screen_option_filter($status, $option, $value) {
//    $value = (int) $value;
